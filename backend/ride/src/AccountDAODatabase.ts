...
export default class AccountDAODatabase implements AccountDAO, SignupAccountDAO, GetAccountAccountDAO {
	...
	async save (account: any) {
		...
		await connection.query("insert into app.account (account_id, name, email, cpf, car_plate, is_passenger, is_driver) values ($1, $2, $3, $4, $5, $6, $7)", [account.accountId, account.name, account.email, account.cpf, account.carPlate, !!account.isPassenger, !!account.isDriver]);
